...
《AirCare × 數據中台架構關係圖》寫「分群引擎：**八分群** `sensor_data.py`」，但 `~/repos/dataspec` 底下找不到這個檔案。

**答案在 v2 規格的 §8 可追溯來源**：分群與評分的程式都在 **`aircare-frontend/mcp_server/tools/`**（`scoring.py`、`sensor_data.py`、`config.py`），不在數據中台、也不在 dataspec —— 那是 AirCare 那一側的 repo，不在 AirSure 這台機器上。

**仍待確認**：架構關係圖寫「八分群」，但 v2 規格與 AirSure 程式碼都是**七分群**（P×H 矩陣，2026-09-03 commit `eea8a1a` 對齊）。若兩邊規則不同，報告上的等級會跟 AirSure 畫面上的分群對不起來。

### 9.2 ⚠ 評分公式的真正權威來源不是程式碼

v2 §7 明定：**生效公式由 PostgreSQL `scoring_config` 表中 `active = TRUE` 的設定決定，程式在執行時讀取，不以硬編碼公式為準。**

這一條很重要，因為它代表：

- **改分數規則不用改程式**，改 DB 設定即可 —— 但也表示光看程式碼無法確定線上實際用哪一版
- 排查分數異常時，第一站是 `scoring_config` 而不是 `scoring.py`
- 2026-09-08 發現的報告濕度分數全為 `0.0`（總分固定低 50 分），最可能就是這張表的 `environmental_humidity_index` 設定或輸入值有問題。詳見 `docs/aircare-0918-報告寄送計畫.md` §1
