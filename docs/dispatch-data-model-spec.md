...
13 項議題中 **12 項已定案**；#7 通路狀態來源保留待討論（§2）。
...
| 8b | 打開報告是次數還是人數 | **兩者都存**：`opens`（事件數）與 `unique_opens`（去重戶數）；**互動率的分母用 `unique_opens`** |
| 9 | 歸因時間窗 | **7 天** |
...
## 二、開放項：#7 通路狀態來源（電子豹）—— 保留待討論

**狀態：2026-09-08 保留，待另行討論如何處理。** 其餘 12 項議題皆已定案（§1）。

### 2.1 這一項卡住什麼、沒卡住什麼

| 漏斗階段 | 來源 | 是否被此項卡住 |
|---|---|:--:|
| 名單 / 可產製 | 中台自有（`campaign_member.eligible`） | 否 |
| **已寄發 / 已送達** | 通路服務商 | **是** |
| 已開啟 | **我們自己的埋點**（決策 6：報告連結被點開） | 否 |
| 點 CTA | 我們自己的埋點（決策 8） | 否 |
| 服務跟進 | SF（決策 9／10） | 否 |

也就是：**六階漏斗裡只有中間兩階被卡住**，其餘四階可以先做。建議不要等這一項才動工。
...
`dispatch.state` 先只落 `queued` / `sent`（我們自己送出的當下即可寫），`delivered` / `bounced` / `unsubscribed` 留 null；前端漏斗的「已送達」一階標「待通路資料源」。通路確定後補寫歷史事件即可，**表結構不用改**。
