...
- 狀態：**項目 1 已於 2026-09-03 實作**（見 `docs/implementation-notes/module-a-v2-alignment-implementation-note.md`）；項目 2、3 仍待確認
...
> ※ 2026-09-03 起，第一個主 tab「整體場域」已更名為「設備總覽」。本文保留當時命名，改動見 `docs/module-a-device-overview-plan.md`。
...
1. ~~**範圍**：只做項目 1，還是 1+2+3 一批做完？~~ → 2026-09-03 使用者裁決**先做項目 1**，項目 2、3 未動。
2. ~~**項目 1b** 分布數字重配是否可接受~~ → 2026-09-03 使用者裁決**重配，且分群由 P×H 矩陣推導**。
3. **項目 3** 的 `dhi` / `AHI` 是否等同 v2「AirCare 分數」
   → 實作時已按「是」處理：`FieldRecord.q` / `FieldDetail.dhi` 一律由 `aircareIndex()` 產出。
   若中台認定兩者不同，需另開欄位。
4. ~~第 7 群 `identity` 文案「空氣偏乾型」是否合適~~ → 已採用。

### 實作時偏離本文之處

| 本文原訂 | 實際做法 | 原因 |
|---|---|---|
| `CategoryMeta.code` 維持 v1 型別名（維持型/穩定型…） | 改為 v2 分群名稱（金級空氣/銀級空氣/…/乾燥） | 使用者 2026-09-03 裁決「用 v2 官方名稱」，與中台 segment 字串同一套語彙 |
| 項目 1 只重配 `CATEGORY_DIST` 數字 | 分群改由 `segmentOf(pm, humidity)` 從 P×H 矩陣推導，母體的 pm / 濕度重新取樣 | 只改數字會出現「標濕度風險、濕度卻 56%」的矛盾列 |
| `AIR_QUALITY_DIST.catIds` 硬綁模型列為「另案」 | 本輪一併拆掉，改由母體即時算出每一級實際落到的分群 | 級距改 v2 後硬綁的 catIds 必然對不上，留著就是壞資料 |
| 未提及分數 | `q` / `dhi` / 詳情頁 `airScore` 一律改用 v2 §4 公式 | 分群改了、分數沒改，同一列會出現「金級空氣 + 狀態異常」 |
