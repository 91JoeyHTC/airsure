...
| 步 | 誰 | 做什麼 | 產出 |
|---|---|---|---|
| ① 挑名單 | 你 | 於場域清單用「可挑選」chip 篩選後勾選 30–60 位 → 匯出 CSV | 名單（客戶編號 + MAC + 訂單號 + 機型 + 地址） |
...
|---|---|---|
| **場域清單**（九態狀態機） | ✅ **已做**：勾選挑名單 + 匯出 CSV（客戶編號 / MAC / 訂單號 / 機型 / 地址 / 感測天數） | ❌ 按鈕觸發產製（九態按鈕已一律停用並標「待接入」） |
...
| 3b | 挑名單忽略輪廓 | ✅ 已決（2026-09-08） | 報告會少「客戶輪廓」個人化段落；中台補 `family_bothered` 後自動放行 |
