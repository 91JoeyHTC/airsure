...
## 2026-09-08 追加：挑名單 + 匯出（為 9/18 首批寄送）

對應：`docs/aircare-0918-報告寄送計畫.md` §8 工作流程第 ① 步

### 項目：場域清單要能挑名單並匯出給報告產生器

**需求來源**：9/18 要手動挑 30–60 位客戶寄報告，但清單上的 checkbox 是 `<input type="checkbox" />`（無 state、無 onChange），右上「匯出」按鈕沒有 onClick —— 挑名單這件事在畫面上做不到。

**目前問題（動工前實測）**

| 現象 | 原因 |
|---|---|
| 用「可產製」篩不到任何真實客戶 | 72 台真實設備全被判 `need-profile`，因為中台尚未回傳 SF `family_bothered` |
| checkbox 點了沒反應 | 純視覺，沒有接 state |
| 匯出按鈕點了沒反應 | 沒有 onClick |
| 列上沒有 MAC | `ReportDeviceRow.code` 是遮蔽格式，`fieldId` 只有已有報告的 3 台才有值 |

**決策（使用者裁決 2026-09-08）**：**挑名單時忽略輪廓**。輪廓缺了報告仍能產，只是少一段個人化內容；等中台補 `family_bothered` 後九態會自動放行（`applyLiveProfiles` 已就緒）。

### 已完成

| # | 改動 | 檔案 |
|---|---|---|
| 1 | `ReportFilterKey` 新增 `'selectable'`，判定為「有任一台設備不是 ① 資料未達標」 | `module-a-report.ts` |
| 2 | 新增 `pickRowsOf()`：把選取客戶展開成一台設備一列，並以訂單號 join 回 `ELIGIBLE_BY_CUSTOMER` 補 MAC 與地址 | 同上 |
| 3 | 選取狀態（以客戶為單位，跨頁保留）、全選作用於「目前篩選結果」而非當頁 | `AFieldList.tsx` |
| 4 | 挑名單摘要條：已選人數／台數、示範客戶警示、清除、匯出 CSV | 同上 |
| 5 | 九態操作按鈕一律 `disabled` + `title="待接入"` | 同上 |
| 6 | ModuleA 右上「匯出」「新增場域」改為 disabled + 說明 | `ModuleA.tsx` |
| 7 | `DetailSource` 橫幅：場域詳情標示資料來源三態 | 同上 |

### 設計決策

**選取以「客戶」為單位**，不是設備 —— 對齊 Dispatch 決策 2「一戶一次寄發，內含多份設備報告」。匯出時才展開成設備列。

**全選作用在「目前篩選結果」而非當頁** —— 一頁 20 列、母體 79 列，只全選當頁會讓人翻頁後以為選丟了。

**CSV 加 UTF-8 BOM** —— 否則 Excel 開啟中文欄位是亂碼。

**示範列保留在匯出中但標 `isDemo=Y` 且 MAC 為空**，摘要條另外警示「含 N 位示範客戶（無 MAC，產報告時請排除）」。直接濾掉會讓使用者以為勾選數與匯出列數對不起來。

**MAC 是內部主鍵**，畫面上一律顯示遮蔽代號（`C..88-A`），只在匯出檔出現；匯出屬對內用途，檔案不得外流（AGENTS.md §7）。
...
|---|---:|---|
| 真實設備報告 | 3 | 無（就是真的） |
| 手工示範（`FIELD_DETAILS`，即 SH-2841 王婉真） | 1 | 黃底「示範場域 —— 手工示範資料，非真實量測」 |
| fallback 骨架（其餘） | 1,280 | 紅底「**曲線為借用** —— 趨勢圖與熱力圖是借第一台真實設備的序列，不代表本場域」 |

第三種是重點：那 1,280 個場域的 PM2.5 趨勢、溫濕度曲線**全是同一條線**（`FIELD_DETAIL_C88 = DEVICE_FIELD_DETAILS[0]`），拿去 demo 會被當成各自的量測結果。

### 驗證紀錄

```
狀態 chip: 全部 79 · 可挑選 78 · 可產製 1 · 待補輪廓 71 · 資料未達標 1 · 逾期待更新 1
「可挑選」= 78 位(真實 69 / 示範 9)   ← 69 位真實客戶全可挑，足夠挑 30–60 位

模擬勾選 5 位真實客戶 → 展開 5 台設備，CSV 內容:
C2026010030,806599912074,01150125014,CS101,台南市,安南區,總安街一段,87,N,N,need-profile
C2026010062,1cdbd4f8def8,01150210006,CS101,台中市,南區,文心南路,87,Y,N,need-profile
...
示範列匯出:mac="" isDemo=true  ✅
```

| 驗證 | 結果 |
|---|---|
| `npm run build` | ✅ 通過（ModuleA chunk 265.32 → 269.48 kB） |
| `npm run lint` | ✅ 9 errors，與改動前相同，未新增 |
| 資料層實跑 | ✅ 篩選數、MAC join、示範列處理皆正確 |
| 瀏覽器人工 smoke | ⚠ blocked（無 playwright） |

### 待人工確認

- [ ] 勾選後摘要條出現，跨頁保留選取
- [ ] 全選勾的是「目前篩選結果」而非當頁
- [ ] 匯出的 CSV 用 Excel 開啟中文不亂碼
- [ ] 九態操作按鈕呈停用且滑過有說明
- [ ] 開啟示範場域的場域詳情，頂端出現紅底「曲線為借用」橫幅

### 下一步

輪廓那條路仍待中台補 `family_bothered`；補上後「可產製」才會有真實客戶，屆時「可挑選」這個過渡篩選可考慮保留（挑名單本來就不該被輪廓擋）或收掉。
