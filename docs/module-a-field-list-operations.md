...
`/module-a` 的第三個主 tab。IA 順序：**整體場域 → 分類概況 → 場域清單 → 個人場域資訊**。
...
chip 可與六大類型篩選並存（類型篩選由「整體場域」的建議聯繫客戶卡片點擊帶入，右側有 ✕ 可清除）。
...
| 設備 | N 台；> 1 台可點展開 ▾ |
| 分群 | 對外只揭露金/銀/銅（風險類型對外一律以銅級揭露），灰字副標才是內部六大類型代號；「待報告產出」= 這台還沒有設備分析報告 |
...
從「整體場域」的建議聯繫客戶卡片點進來，清單會自動套用該六大類型篩選；篩選 chip 右側 ✕ 清除。
