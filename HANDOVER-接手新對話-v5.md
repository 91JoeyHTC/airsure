...
- **目前唯一存在的 `report_type` 是季報**，全名單皆以季報寄發；週／月報是另一種 report_type，內容與 CTA 尚未定義，卡片只顯示預分派規模（471 / 84 戶）並標「規劃中」，不給成效。上線時要改回來的兩處程式碼內已標註。
- 漏斗有「可產製」一階：未達 90 天門檻的客戶**仍留在名單內**，流失顯示在該階。
- CTA 口徑：互動率 = 點擊數 ÷ 打開報告數；服務跟進歸因為 7 天窗、last-click。
- **9/18 首批報告寄送計畫**：`docs/aircare-0918-報告寄送計畫.md` —— **給接手工程師的交接規格**（30–60 位手動挑選 · LINE／簡訊／Email 三通路 · 報告站需獨立網域、平台中立需求 + 三個參考選項 · 不走中台）。
  ⚠ 內含一個阻斷級問題：現行 AirCare 報告的**濕度分數全被算成 0.0**，依 v2 §4.2 平均濕度 46.8–59.5% 應得 100 分，總分因此**固定低 50.0 分**（權重 0.5）。分群則正確 —— 只壞評分。寄送前必修，查 `scoring_config` 的 `environmental_humidity_index`。
- 資料模型規格：`docs/dispatch-data-model-spec.md`（13 項議題中 **12 項已定案**；僅 #7 通路狀態來源／電子豹保留待討論，它只卡住漏斗的「已寄發／已送達」兩階，其餘四階可先接）。
